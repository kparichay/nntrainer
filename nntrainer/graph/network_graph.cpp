...
int NetworkGraph::compile(const LossType loss_type) {
  ...
  for (unsigned int i = 0; i < adj.size(); ++i)
     ...
     */
    // FIXME: THIS HAS SOME ISSUE
    // adj[i].resize(1, std::shared_ptr<LayerNode>(nullptr, 0));
    adj[i].resize(1);
...
int NetworkGraph::addLossLayer(const LossType loss_type) {
  ...
  if (updated_loss_type == LossType::LOSS_ENTROPY) {
    if (last_node->getObject()->getType() != "activation") {
...
void NetworkGraph::setOutputLayers() {
  ...
  for (unsigned int idx = 0; idx < adj.size(); ++idx) {
    ...
    std::cout << layer_idx->getName() << std::endl;
    for (unsigned int i = 0; i < adj.size(); ++i) {
...
std::shared_ptr<LayerNode> &NetworkGraph::getLayerNode(const std::string &layer_name) {
